<template>
    <div style="padding:16px;">
      <el-form inline class="searchBar">
        <el-form-item label="建议主题:">
            <el-input size="small" v-model="searchValue" maxlength="10" placeholder="输入建议主题" style="width:110px;"></el-input>
        </el-form-item>
      ...
      <el-button @click="openAddView" type="primary" size="mini" icon="el-icon-plus" style="margin-top: 20px;margin-left: 10px;">建议</el-button>
      <!-- 分页 -->
      <Pagination class="pagination" v-bind:child-msg="pageMsg" @pageChange="pageChangeHandle"></Pagination>

        <!-- 建议信息 -->
        <el-dialog title="我的建议信息" :visible.sync="adviceInfoVisible" width="60%" center>
            <el-descriptions :column="2" style="display: inline-block;">
                <el-descriptions-item label="建议主题">
                    <el-input v-if="editForm.status==0" v-model="editForm.subject" placeholder="请输入建议主题" maxlength="50" size="mini" style="width: 250px;"></el-input>
                ...
                <el-descriptions-item label="提交时间" span="2">{{editForm.createTime}}</el-descriptions-item>
            ...
            <div style="width: 150px;margin-left: auto;margin-right: auto;">
                <el-button :disabled="editForm.status!=0" type="primary" size="mini" @click="editAdvice">保存</el-button>
                <el-button size="mini" @click="adviceInfoVisible=false">取消</el-button>
            </div>
        </el-dialog>

        <!-- 添加建议 -->
        <el-dialog title="我要建议" :visible.sync="addViewVisible" width="60%" center>
            <el-form ref="addForm" :model="addForm" label-width="80px" size="mini">
                <el-form-item label="建议主题">
                    <el-input v-model="addForm.subject" maxlength="50" placeholder="请输入建议主题"></el-input>
                </el-form-item>
                <el-form-item label="建议类型">
                    <el-select v-model="addForm.type" placeholder="请选择建议类型">
                        <el-option label="保洁卫生" :value="1"></el-option>
                        <el-option label="安保" :value="2"></el-option>
                        <el-option label="质量" :value="3"></el-option>
                        <el-option label="服务态度" :value="4"></el-option>
  ...
  export default {
      ...
      methods:{
          handleEdit(row) {
              this.adviceInfoVisible = true;
              Object.assign(this.editForm,row);
          },
          handleDelete(row) {
              this.$confirm('是否删除此建议信息?', '删除建议提示', {
          ...
          pageChangeHandle(param) {
            ...
            this.myAdvice(currentPage,subject,type,status)
          },
          openAddView() {
              this.addViewVisible = true;
          },
          myAdvice(currentPage,subject,type,status){
            let page = this
            let token = this.$store.getters.GET_TOKEN
            let url = "/heng/advise/myAdvice"
            let params = {
                "subject": subject,
                "type": type,
                "status": status,
          ...
          deleteAdvice(id){
            let page = this
            let token = this.$store.getters.GET_TOKEN
            let url = '/heng/advise/deleteAdvice'
            let params = {
                'id': id
            }
            this.$http.post(url, params, {headers:{'token':token}}).then(function(res){
          ...
          editAdvice(){
            let page = this
            let token = this.$store.getters.GET_TOKEN
            let url = "/heng/advise/editAdvice"
            let params = {
                "id": this.editForm.id,
